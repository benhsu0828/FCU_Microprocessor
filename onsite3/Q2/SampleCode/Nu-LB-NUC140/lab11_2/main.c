...
void getVR()
{
    ...
    if (u32ADCvalue * 96 / 4096 != pad_x)
    {
        ...
        {
            UART_WRITE(UART0, TX_buffer[forCount]);
        }
...
void judgeup()
{
    if (ball_x > 96 - ypad_x && ball_x < 96 - ypad_x + 32)
    {
        touchFlag = 1;
        digit[1]++;
        if (digit[1] == 10)
...
void judgedown()
{
    if (ball_x > pad_x && ball_x < pad_x + 32)
    {
        ...
        PWM_DisableOutput(PWM1, PWM_CH_0_MASK);
        PWM_ConfigOutputChannel(PWM1, PWM_CH0, 1, 20);
        PWM_EnableOutput(PWM1, PWM_CH_0_MASK);
    }
}
        ...
        if (resultFlag == 0)
        {
            if (cnt0_5ms % 20 == 0)
            {
                ...
                if (ball_x > 120)
                {
                    ball_x = 120;
                    ball_dirx = -1;
                    PWM_ConfigOutputChannel(PWM1, PWM_CH0, 2784, 20);
                    PWM_EnableOutput(PWM1, PWM_CH_0_MASK);
                }
                if (ball_x < 0)
                {
                ...
                draw_bitmap_in_buffer(pad, 96 - ypad_x, ypad_y, 32, 1, FG_COLOR);
                show_lcd_buffer();
            }
