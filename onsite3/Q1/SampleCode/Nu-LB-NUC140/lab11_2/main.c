...
volatile int ball_vx = 4;
volatile int ball_vy = 4;
...
void Init_ALL(void)
{
    ...
    //PWM_ConfigOutputChannel(PWM1, PWM_CH0, 523, 50);
    ...
    while (1)
    {
        ...
        if (resultFlag == 0)
        {
            ...
            if (cnt0_5ms % 20 == 0)
            {
                ...
                //draw_Bmp32x8(0,0 ,BG_COLOR,BG_COLOR , pad);
                //draw ball
                //draw_Bmp8x8(ball_x, ball_y,BG_COLOR,BG_COLOR , ball);
                if (touchFlag == 0)
                {
                    if (cnt0_100ms % 2 == 0)
                        PWM_DisableOutput(PWM1, PWM_CH_0_MASK);
